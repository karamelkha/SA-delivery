...
FR-16. При неуспешной оплате система должна сохранять статус оплаты «Оплата не прошла» и оставлять заказ в статусе «Ожидает оплаты» до истечения времени ожидания оплаты, без перевода в статус «Заказ не оплачен» до срабатывания таймера.
...
### 3.9 Пост-обработка оплаченных заказов в статусах «Отклонён» и «Отменён»

FR-42. При переводе оплаченного заказа в статус «Отклонён» или «Отменён» система должна формировать событие `RefundRequired` для запуска внешнего процесса возврата.

FR-43. Система должна передавать во внешний процесс возвратов минимально необходимый набор данных: `order_id`, `payment_id`/`provider_payment_id`, сумму, причину инициирования возврата, дату и время события.

FR-44. В рамках текущего жизненного цикла заказа статусы «Отклонён» и «Отменён» должны считаться финальными; результат внешнего процесса возврата не должен изменять статус заказа.

FR-45. Система должна фиксировать в истории заказа события интерфейсного взаимодействия с процессом возврата (`RefundRequired`, `RefundCompleted`, `RefundFailed`) без изменения финального статуса заказа.

---

## 4. Ограничения

- Возвраты и частичные возвраты платежей находятся вне рамок проекта и рассматриваются как внешний процесс, с которым задана интерфейсная граница.
