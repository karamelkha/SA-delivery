...
3. Заказ остаётся в статусе «Ожидает оплаты» до истечения установленного времени ожидания оплаты (без перевода в статус «Заказ не оплачен» до срабатывания таймера).
...
4. Для оплаченного заказа система формирует событие `RefundRequired` и передаёт данные во внешний процесс возврата.
...
- В случае отмены оплаченного заказа система формирует событие `RefundRequired` и передаёт данные во внешний процесс возврата.
...
5. Администратор принимает решение:
   ...
   - отменить заказ.
6. При отмене оплаченного заказа система формирует событие `RefundRequired` и передаёт данные во внешний процесс возврата.
...
## UC-7. Инициировать пост-процесс возврата оплаты

**Цель:** передать данные для возврата оплаты во внешний процесс возвратов для оплаченного заказа в статусе «Отклонён» или «Отменён».  
**Основной актор:** Система  
**Вторичные акторы:** Внешний процесс возвратов

### Предусловия

- Заказ имеет успешную оплату.
- Заказ переведён в статус «Отклонён» или «Отменён».

### Триггер

Смена статуса оплаченного заказа на «Отклонён» или «Отменён».

### Основной поток

1. Система формирует событие `RefundRequired`.
2. Система подготавливает данные для внешнего процесса возвратов: `order_id`, `payment_id`/`provider_payment_id`, сумма, причина, `event_timestamp`.
3. Система передаёт данные во внешний процесс возвратов.
4. Система фиксирует факт отправки запроса в истории заказа.
5. При получении ответа внешний результат фиксируется как событие `RefundCompleted` или `RefundFailed`.

### Альтернативные потоки

**A1. Недоступен внешний процесс возвратов**

1. На шаге 3 внешний процесс возвратов недоступен.
2. Система фиксирует событие ошибки интеграции в истории заказа.
3. Система инициирует повторную попытку передачи запроса по внутренним правилам обработки интеграционных ошибок.

### Постусловия

- Запрос на возврат передан во внешний процесс или зафиксирована ошибка интеграции.
- Статус заказа в рамках текущего жизненного цикла не изменяется и остаётся «Отклонён» или «Отменён».
