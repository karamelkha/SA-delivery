...
9. Клиент инициирует оплату заказа через платёжный сервис.
...
2. Система не изменяет статус оплаты заказа.
3. Заказ остаётся в статусе «Ожидает оплаты» до истечения установленного времени ожидания оплаты.
...
5. После завершения приготовления заказа система обновляет статус заказа на «Готов к доставке».
...
**Цель:** назначить курьера для доставки заказа.  
...
Система инициирует назначение курьера для доставки заказа.
...
2. Курьер переводит доставку заказа в статус «В пути».
3. Система сохраняет статус доставки «В пути» и фиксирует изменение в истории изменений заказа.
4. После завершения доставки курьер переводит доставку заказа в статус «Доставлено».
5. Система сохраняет статус доставки «Доставлено», фиксирует факт завершения доставки и обновляет статус заказа на «Доставлен».
6. Обновлённый статус заказа отображается клиенту и администратору.

### Альтернативные потоки

**A1. Не удалось доставить заказ**

1. После перевода доставки в статус «В пути» курьер не может завершить доставку.
2. Курьер выбирает действие «Не удалось доставить» и указывает причину из доступного списка.
3. Система фиксирует событие «Доставка не выполнена» и сохраняет указанную причину.
...
**A2. Ошибка при сохранении статуса доставки**

1. На шаге 3 или 5 возникает ошибка при сохранении статуса доставки.
...
4. Текущий статус доставки и статус заказа в системе остаются без изменений до успешного сохранения.
