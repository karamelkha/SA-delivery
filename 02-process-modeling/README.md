...
- завершение заказа в статусе «Доставлен», «Отклонён» либо «Отменён».

Диаграмма отражает все ключевые переходы статусов, определённые в `Order_Statuses.md`.

Для оплаченных заказов, завершившихся в статусах «Отклонён» или «Отменён», сквозной процесс формирует событие `RefundRequired` и передаёт данные во внешний процесс возвратов (процесс возвратов находится вне рамок данного проекта).
...
- обработка неуспешной оплаты или технической ошибки без изменения статуса заказа (заказ остаётся в статусе «Ожидает оплаты» и может быть повторно оплачен до истечения таймера);
- перевод заказа в статус «Заказ не оплачен» только при истечении лимита времени оплаты;
- сохранение результата оплаты.

Платёжный сервис моделируется как внешняя система (black box) с использованием message events и message flows.
...
Подпроцесс завершает выполнение, возвращая в основной процесс один из следующих результатов:
...
## 8. Интерфейсная граница с процессом возвратов (out of scope)

В BPMN-границах текущего проекта процесс возвратов не моделируется как внутренний подпроцесс. Вместо этого определяется внешняя граница интеграции:

- событие-инициатор: `RefundRequired`;
- условия генерации: оплаченный заказ переведён в статус «Отклонён» или «Отменён»;
- передаваемые данные: `order_id`, `payment_id`/`provider_payment_id`, сумма, причина, `event_timestamp`;
- ожидаемые ответы внешнего процесса: `RefundCompleted` или `RefundFailed` (фиксируются в истории заказа).
